...
try {
  const indexFileExists = fs.existsSync(
    path.join(__dirname, '../pages/forum/index.vue')
  )
...
try {
  ...
  assert.strictEqual(
    fileContent.includes('<main class="main-content">'),
    true,
    '文件应该包含main-content类'
  )
  assert.strictEqual(
    fileContent.includes('<div class="topics-list">'),
    true,
    '文件应该包含topics-list类'
  )
  assert.strictEqual(
    fileContent.includes('topics = ref('),
    true,
    '文件应该包含topics数据定义'
  )
