...
console.log(
  `${colors.magenta}      论坛页面 (Forum Index) 测试套件      ${colors.reset}`
)
console.log(
  `${colors.blue}==========================================${colors.reset}\n`
)
...
passed += runTest('状态管理检查', () => {
  ...
  assert.strictEqual(
    indexVueContent.includes('const toggleDropdown = (name)'),
    true
  )
})
...
passed += runTest('话题数据结构检查', () => {
  const scriptPart = indexVueContent
  ...
  assert.strictEqual(
    scriptPart.includes('我的 2024 - 稳中求进、热爱生活'),
    true
  )
...
if (
  indexVueContent.includes('<template>') &&
  indexVueContent.includes('</template>')
) {
...
passed += runTest('主要布局结构检查 (使用完整文件内容)', () => {
  assert.strictEqual(
    indexVueContent.includes('class="main-content"'),
    true,
    '应包含main-content类'
  )
  assert.strictEqual(
    indexVueContent.includes('class="content-header"'),
    true,
    '应包含content-header类'
  )
  assert.strictEqual(
    indexVueContent.includes('class="topics-list"'),
    true,
    '应包含topics-list类'
  )
})
...
passed += runTest('组件和指令检查 (使用完整文件内容)', () => {
  // 检查组件使用
  assert.strictEqual(
    indexVueContent.includes('NuxtLink'),
    true,
    '应包含NuxtLink组件'
  )
  assert.strictEqual(
    indexVueContent.includes('Dropdown'),
    true,
    '应包含Dropdown组件'
  )
  ...
  // 检查指令使用
  assert.strictEqual(
    indexVueContent.includes('v-for="topic in topics"'),
    true,
    '应包含topics循环'
  )
  assert.strictEqual(
    indexVueContent.includes(':key="topic.id"'),
    true,
    '应包含key绑定'
  )
  assert.strictEqual(
    indexVueContent.includes('@click="toggleDropdown'),
    true,
    '应包含click事件绑定'
  )
})
...
passed += runTest('样式引入检查', () => {
  const stylePart = indexVueContent
