export const useForumApi = () => {
  ...
  return {
    ...
    // 获取所有帖子列表
    getPostsList: (method, start, limit) =>
      nuxtApp.$axios.get(
        `/forum/posts?limit=${limit}&start=${start}&method=${method}`
      ),
    ...
    // 发表新帖子
    createNewPost: (title, content, tags = []) =>
      nuxtApp.$axios.post('/newpost', {
        data: {
          title,
          tags,
          content,
    ...
    getPostContent: async (postId, userId = null) => {
      if (!userId) return await nuxtApp.$axios.get(`/forum/posts/${postId}`)
        ...
        return await nuxtApp.$axios.get(
          `/forum/posts/${postId}?user_id=${userId}`
        )
    },

    // 获取用户收藏的帖子列表
    getFavoritePostsList: userId =>
      nuxtApp.$axios.get(`/forum/favorite?user_id=${userId}`),

    // 获取用户发布的帖子列表
    getUserPostedList: userId =>
      nuxtApp.$axios.get(`/user/profile/${userId}/posts`),
